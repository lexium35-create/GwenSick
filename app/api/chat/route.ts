...
import { createSeekAIProvider } from "@/lib/ai/seekai";
...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: "GwenSick is not configured yet. Set SEEKAI_API_KEY on the server." },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    const payload = body as { messages?: unknown; mode?: unknown; model?: unknown };
    ...
    if (!selectedMessages.every(isValidMessage)) {
      return NextResponse.json({ error: "Each message must have a valid role and non-empty content within the size limit." }, { status: 400 });
    }
    ...
    const provider = createSeekAIProvider({
      apiKey,
      model,
      mode,
      modePrompt: MODE_PROMPTS[mode],
    });

    const response = await provider.respond(messages);
    return NextResponse.json({ message: response, mode, model });
  } catch (error) {
